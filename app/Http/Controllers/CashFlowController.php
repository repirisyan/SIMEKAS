...
class CashFlowController extends Controller
{
    ...
    public function store(Request $request){
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $filename = rand().'.webp';
                $originalImage = Image::load($request->file('image'))->format(Manipulations::FORMAT_WEBP)->optimize()->save(storage_path('app/public/cashflow/original/'.$filename));
                $thumbnailImage = Image::load($request->file('image'))->format(Manipulations::FORMAT_WEBP)->width(100)->height(100)->optimize()->save(storage_path('app/public/cashflow/thumbnail/'.$filename));
            }
            ...
            return to_route('cashflow.index')->with('message', array('error','Terjadi kesalahan saat menyimpan data'));
        }
    }
    ...
    public function destroy($id){
        try {
            ...
            return to_route('cashflow.index')->with('message',array('success','Data berhasil dihapus'));
        } catch (Exception $e) {
            return to_route('cashflow.index')->with('message',array('error','Terjadi kesalahan saat menghapus data'));
        }
    }
}
